...
>
> 当前这会落地为干净的 Pull Request;而北极星是把每个 Task 送完最后一程——**合并与
> CI/CD**——让一个 workspace *上线*,而不只是开出 PR。
...
| **终点** | 无界 | **Task → 上线**——当前开 PR,**合并 + CI/CD** 是目标 |
...
**自动合并与 CI/CD**——让"完成"的单位是上线的代码,而不是开出的 PR。这是路线图,而非
当前行为。
