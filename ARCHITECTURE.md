...
- **UI**:已实现 workspace board、repo map、thread Lead/Board、write-scope review、worker session、Observe/Diff、Needs-you、Settings、Inspect、Command Palette、First Run Onboarding、明暗主题、中英 UI 和 agent 输出语言偏好。
...
- **黑板按 thread**:每个 thread 有自己的 proposal/brief/bus state;团队 skills/rules 仍 workspace 级共享。
...
- M6:两级看板、Needs-you、repo contention、thread bus/coordinator、i18n、settings、有效配置预览。已实现基础版;团队 marketplace 下发、PR/CI/CD 观测仍待做。
